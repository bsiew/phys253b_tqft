...
Every QFT student knows what a global symmetry is: a conserved current $j^\mu$, a charge $Q = \int_\Sigma \star j$, and a Ward identity.  But there is a more revealing reformulation.  Exponentiate the charge into an operator $U_g(\Sigma) = e^{i\alpha Q_\Sigma}$ supported on a codimension-1 surface $\Sigma$.  Current conservation guarantees that $U_g(\Sigma)$ is \emph{topological}---deformable without changing correlators, provided the deformation does not cross a charged insertion.

Once you see an ordinary symmetry this way, the generalization writes itself: change the dimension of the charged object.  A Wilson line is one-dimensional; the symmetry operator that detects it must be codimension-2.  That is a 1-form symmetry.  The pattern continues to $q$-form symmetries for any $q$, with the same Stokes-theorem logic at each level \cite{GKSW2015,Bhardwaj2023Lectures}.  This chapter develops that language, then uses it to make anomaly inflow---the mechanism by which bulk topological terms constrain boundary degrees of freedom---precise and computable.

\section{Ordinary \texorpdfstring{$0$}{0}-form symmetries in defect language}\label{sec:zero-form-defects}

The key claim is:
\begin{quote}
an ordinary global symmetry is a topological operator supported on a codimension-$1$ manifold.
\end{quote}
For ordinary symmetries this is not a new postulate---it is Noether's theorem rewritten in a form that generalizes to higher dimensions.
...
The $(d-1)$-form $\star j$ can be integrated only over $(d-1)$-dimensional submanifolds. The operator $U_g(\Sigma)$ therefore lives on a codimension-$1$ support.  (This is the only place where "codimension-1" needs to be remembered as a definition; everything else follows from it by shifting dimensions.)
\end{remark}
...
The step to higher-form symmetries is purely dimensional.  Replace point operators by extended operators, keep the same topological-deformation logic, and track which dimensions go where: a $q$-form symmetry acts on $q$-dimensional operators and is implemented by a topological defect of codimension $q+1$ \cite{GKSW2015,Bhardwaj2023Lectures}.  Nothing else changes.
...
So far the symmetry operators have been probes---background data that we couple to the theory and deform to detect charges.  Gauging means promoting that background to dynamical data and summing over it in the path integral.  For higher-form symmetries the logic is identical to the ordinary case; only the degree of the gauge field changes \cite{GKSW2015,Bhardwaj2023Lectures}.
...
When can you gauge a symmetry, and when can't you?  The obstruction has a precise name: an 't~Hooft anomaly.  Operationally, couple the theory to a background field, perform a background gauge transformation, and check whether the partition function is unchanged.  If it shifts by a local functional of the background that no counterterm can remove, the symmetry is anomalous and gauging is obstructed \cite{GKSW2015,Bhardwaj2023Lectures}.
...
An anomaly is not a disease---it is a boundary symptom of a bulk topological response.  Place the anomalous theory on the boundary of a $(d+1)$-dimensional bulk whose own gauge variation produces precisely the opposite phase: the two cancel, and the combined system is gauge-invariant.  The anomaly has not been removed; it has been \emph{explained} as the boundary avatar of a bulk invertible TQFT \cite{CallanHarvey1985,Freed1995CSnotes,FreedMooreTeleman2024}.
...
The general inflow mechanism is now concrete: we connect it to the axial anomaly from Schwartz's Chapter~30.  The cleanest match is the axial anomaly of a four-dimensional Dirac fermion in an external gauge field.  The inflow bulk is a \emph{mixed} five-dimensional Chern--Simons-type term involving the axial background field and the vector field strength---narrower than the full nonabelian $5$-form, but exactly what matches Schwartz's equation with no convention drift.
...
\noindent Part~II now turns to the physical realization of this machinery in gapped phases of matter. Chapter~\ref{sec:topological-order} constructs the toric code as a lattice model whose topological ground-state degeneracy and mutual braiding phase $(-1)$ are exactly the data predicted by the $\Z_2$ BF theory and anomaly-inflow structure developed in Chapters~\ref{ch:bf}--\ref{ch:gensym}.
