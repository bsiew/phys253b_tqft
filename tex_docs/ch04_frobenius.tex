...
How much data does a topological field theory actually contain?  In two dimensions, the answer is surprisingly little: a single finite-dimensional vector space equipped with a multiplication, a unit, and a trace form satisfying a compatibility condition.  That structure---a commutative Frobenius algebra---\emph{is} the theory.  Every 2D oriented TQFT determines one, and conversely every commutative Frobenius algebra determines a 2D TQFT.  The classification is complete \cite{Kock2004Frobenius}.

Physically, the classification has teeth: the ground-state degeneracy of a 2D topological phase on a surface $\Sigma_g$ is determined by the Frobenius algebra of the underlying TQFT. In particular, the $N^{2g}$ degeneracy formula that reappears throughout Part~II---in $\Z_N$ BF theory (Chapter~\ref{ch:bf}), untwisted Dijkgraaf--Witten theory (Chapter~\ref{ch:dw}), and the toric code (Chapter~\ref{sec:topological-order})---is already fixed by the algebraic data of this chapter. The classification thus provides the universal bookkeeping behind topological order in two spatial dimensions.

The reason the classification works is topological.  Every closed oriented 1-manifold is a disjoint union of circles, and every oriented 2-cobordism can be cut (via a Morse function) into a finite list of elementary pieces: caps, cups, and pairs of pants.  The geometric classification problem becomes a generators-and-relations problem in algebra---and that problem is solvable.

\section{Generators and relations of \texorpdfstring{$\mathrm{Cob}_2$}{Cob2}}\label{sec:cob2-generators}

There is only one connected closed oriented 1-manifold: the circle.  So the object side of $\mathrm{Cob}_2$ is trivial---every object is a disjoint union of circles.  The real question is: which cobordisms between circles are the building blocks, and what relations do they satisfy?
    ...
    \IfFileExists{figures/cobordism_generators.pdf}{%
        ...
        \fbox{%
            \parbox{0.88\linewidth}{%
                ...
                \small Final hand-drawn scan to be committed as \texttt{figures/cobordism\_generators.pdf} per the figure register.%
            }%
        }%
    }
...
\noindent The punchline: these relations are not only necessary but sufficient.  A 2-dimensional oriented TQFT \emph{is} a commutative Frobenius algebra---nothing more, nothing less.

\section{The classification theorem}\label{sec:frobenius-classification}

The most economical algebraic package uses only multiplication, unit, and a trace functional---the comultiplication $\Delta$ is then recovered from the nondegenerate pairing defined by that trace.  (This is a general pattern in finite-dimensional algebra: once you have a nondegenerate bilinear form, the dual structure comes for free.)
...
\noindent The theorem is useless without examples.  The two families below make the classification concrete and provide templates for the same structures when they reappear in Chapters~\ref{ch:bf} and \ref{ch:dw}.

\section{Worked examples and the finite-group foreshadowing}\label{sec:frobenius-examples}

Two families of commutative Frobenius algebras will do most of the work in this paper.  The first is a polynomial quotient ring; the second is a finite-group algebra that foreshadows Dijkgraaf--Witten theory.
...
\noindent With these examples in hand, we can now derive the most powerful output of the classification: a closed-form expression for the partition function on \emph{any} closed surface, in terms of a single algebraic object called the handle operator.

\section{Partition functions on closed surfaces of genus \texorpdfstring{$g$}{g}}\label{sec:genus-g-partition}

How does the partition function depend on genus?  The answer is remarkably clean: adding a topological handle multiplies the state by a fixed element $e \in V$, and the genus-$g$ partition function is just $\epsilon(e^g)$.  We now derive this for a closed oriented surface
...
\noindent That completes the 2-dimensional classification.  (Appendix~\ref{app:frobenius} holds the full Morse-theoretic well-definedness proof for the converse direction and the deferred handle-spectrum calculations.)  The payoff for later chapters: the genus formula \eqref{eq:genus-semismple} and the gauge-theory normalization of Remark~\ref{rem:normalization-gauge} will reappear as the state-counting structure of the toric code in Chapter~\ref{sec:topological-order}, where $N^{2g}$ becomes the topological ground-state degeneracy on a genus-$g$ surface.  In three dimensions the Schwarz-type example par excellence is Chern--Simons theory---the subject of the next chapter.
