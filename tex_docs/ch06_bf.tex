...
What is the simplest topological gauge theory you can write in any dimension?  Chern--Simons requires three dimensions and a trace; Dijkgraaf--Witten requires a finite group.  BF theory requires only two differential-form fields and a wedge product: the action $\frac{k}{2\pi}\int B\wedge dA$ is metric-free, parity-even, and makes sense in every dimension $d \geq 2$.  Better still, in the compact abelian case at integer level $N$ it reduces on-shell to $\Z_N$ gauge theory---the continuum description of the toric code.  This makes BF theory the natural bridge from the continuum topological actions of Chapters~\ref{ch:chern-simons} and~\ref{ch:dw} to the topological-order discussion in Chapter~\ref{sec:topological-order} \cite{Horowitz1989,BlauThompson1991}.
...
\noindent Both fields are flat on-shell.  (This is the hallmark of a topological theory: no local excitations, no propagating degrees of freedom---only global holonomy data survives.)
...
What happens when we compactify and set the level to a positive integer?  The $B$ field becomes a Lagrange multiplier that projects $A$ onto flat connections with $\Z_N$ holonomies.  The continuous gauge theory has reduced itself to a discrete one.  We specialize to the compact theory at integer level
\begin{equation*}
k=N\in \Z_{>0}.
\end{equation*}
...
The observable that makes BF theory physically interesting is the \emph{mixed} correlator: one line operator built from $A$, another from $B$, linked in space.  Their expectation value is a pure phase determined by the linking number---exactly the mutual statistics of $e$ and $m$ particles in the toric code.
...
(-1)^{\mathrm{Lk}(C,C')}.
\end{equation*}
This is the continuum precursor of the mutual minus sign acquired when the electric and magnetic quasiparticles of the toric code braid around one another. Chapter~\ref{sec:topological-order} will return to exactly that phase from the lattice-model side.
\end{remark}

\section{Ground-state degeneracy on \texorpdfstring{$\Sigma_g$}{Sigma\_g}}\label{sec:bf-gsd}

How many ground states does BF theory have on a genus-$g$ surface?  In abelian Chern--Simons, each handle contributed one pair of conjugate holonomies, giving dimension $k^g$.  BF theory has \emph{two} independent flat gauge fields, so each handle contributes two pairs, and the dimension grows as $N^{2g}$.  This is the topological ground-state degeneracy of $\Z_N$ gauge theory---the same $N^{2g}$ that the toric code produces on a genus-$g$ lattice (Chapter~\ref{sec:topological-order}).
...
\end{equation*}
so there are exactly $N^{2g}$ holonomy sectors.
\end{remark}

\medskip

\noindent The same $N^{2g}$ will reappear twice more: once from the finite-group path integral in Chapter~\ref{ch:dw}, and once from the lattice Hamiltonian in Chapter~\ref{sec:topological-order}.  That three independent derivations---continuum BF, discrete Dijkgraaf--Witten, and lattice toric code---produce the same degeneracy is the content of Proposition~\ref{prop:toric-dw}.  The next chapter now strips the gauge group down to a finite group and classifies the resulting theories by group cohomology $H^3(BG,U(1))$.
