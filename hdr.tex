...
\title{The Hodge-de Rham spectral sequence}
\author{Brian Siew}
\date{\today}
...
\begin{abstract}
We explain the spectral sequence $H^p\left(X,\Omega^q\right)\Rightarrow H^{p+q}(X,\C)$ valid for complex manifolds, following Deligne, Fr\"olicher, and others. We then explain why it degenerates for compact K\"ahler manifolds, and give examples of the failure of degeneration in other settings.
\end{abstract}
...
\section{Review of complex manifolds and spectral sequences}


\subsection{Complex manifolds and the Dolbeault cohomology}
...
\begin{definition}
    ...
    $$
    H^{p,q}_{\bar\partial}(X) := H^q\left(\mathcal{A}^{p,\bullet}(X), \bar\partial\right) = \frac{\ker \left(\bar\partial:\mathcal{A}^{p,q}(X) \to \mathcal{A}^{p,q+1}(X)\right)}{\image \left(\bar\partial:\mathcal{A}^{p,q-1}(X) \to \mathcal{A}^{p,q}(X)\right)}
    $$
\end{definition}
...
$$
F^pH^n\left(K^\bullet\right):= \image \left(H^n\left(F^pK^\bullet\right) \to H^n\left(K^\bullet\right)\right)
$$
...
\item \textbf{The $E_1$ page:} We turn the page by taking cohomology: $E_1^{p,q} = \ker d_0 / \image d_0$. The original differential $d$ now induces ``horizontal'' arrows $d_1: E_1^{p,q} \to E_1^{p+1, q}$. 
\[\begin{tikzcd}
...
\item \textbf{The $E_2$ page and beyond:} Again, we take cohomology: $E_2^{p,q} = \ker d_1 / \image d_1$. The arrows now moves two steps to the right and one step down: $d_2: E_2^{p,q} \to E_2^{p+2, q-1}$. In general, the $r^\text{th}$ page differential (for a cohomological spectral sequence) goes $r$ steps to the right and $r-1$ steps down: $d_r: E_r^{p,q} \to E_r^{p+r, q-r+1}$.
\[\begin{tikzcd}
...
\begin{theorem}
    ...
    \begin{enumerate}[i.]
        ...
        \item The $E_{r+1}$ page is determined by the cohomology of the $E_r$ page. Concretely, $E_{r+1}^{p,q} = \ker\left(d_r:E_r^{p,q}\to E_r^{p+r,q-r+1}\right)/\image \left(d_r:E_r^{p-r, q+r-1}\to E_r^{p,q}\right)$. 
...
\begin{proof}
    ...
    \begin{enumerate}[i.]
        ...
        $$
        Then, since $\ell \geq n=p+q \implies p-\ell \leq -q \leq 0$, so $Z_\ell^{p-\ell,q+\ell-1} = \left\{x\in F^{p-\ell}K^{n-1} =K^{n-1}\mid dx\in F^p K^{n}\right\}$. Now, we apply $d$ and this gives us exactly $dZ_\ell^{p-\ell,q+\ell-1}=F^pK^n\cap \image (d)$. Thus, we have
        \begin{align*}
        E_{\ell+1}^{p,q} &:= Z_{\ell+1}^{p,q} /B_{\ell+1}^{p,q} \\
        &= Z_{\ell+1}^{p,q} /\left(Z_{\ell}^{p+1,q-1} + dZ_{\ell}^{p-\ell,q+\ell-1}\right) \\
        &= \ker \left(d:F^pK^n\to F^pK^{n+1}\right) / \left(\ker \left(d:F^{p+1}K^n\to F^{p+1}K^{n+1}\right) +F^pK^n\cap \image (d)\right) \\
        &= \image \left(H^n\left(F^pK^\bullet\right)\to H^n\left(K^\bullet\right)\right)/\image \left(H^n\left(F^{p+1}K^\bullet\right)\to H^n\left(K^\bullet\right)\right) \\
...
\section{The Hodge filtration and the Fr\"olicher spectral sequence}
...
\begin{align*}
E_1^{p,q} &= \ker \left(d_0:E_0^{p,q} \to E_0^{p,q+1}\right) / \image \left(d_0: E_0^{p,q-1}\to E_0^{p,q}\right) \\
&= \ker\left(\bar\partial: \mathcal{A}^{p,q}(X)\to \mathcal{A}^{p,q+1}(X) \right) / \image \left(\bar\partial: \mathcal{A}^{p,q-1}(X)\to \mathcal{A}^{p,q}(X) \right)
\end{align*}
...
\section{The Hodge decomposition and degeneration for compact K\"ahler manifolds}
...
\section{Failure of degeneration}
\subsection{The Iwasawa manifold}
...
\nocite{Huybrechts2005,VoisinI2002,VoisinII2003,Frolicher1955,Lang2002}
