...
Chapter~\ref{ch:bf} showed that compact abelian BF theory at level $N$ reduces to $\Z_N$ gauge theory. Dijkgraaf--Witten theory starts from that discrete point of view directly: the gauge group is finite from the outset, and the topological action is no longer written as a differential-form integral but as a group-cohomological phase. This makes Dijkgraaf--Witten theory the cleanest finite-group counterpart to Chern--Simons and BF theory. It has no local propagating degrees of freedom, its partition function is a finite sum, and its twisted sectors provide a concrete laboratory for topological response and anomaly inflow \cite{DijkgraafWitten1990}.
...
We now specialize the general formalism to the finite cyclic group. This is the case that will later connect back to Chapter~\ref{ch:bf}, where compact abelian BF theory at level $N$ reduced to $\Z_N$ gauge theory. The main text needs two concrete facts:
\begin{enumerate}
...
We now isolate the most important concrete case for the rest of the paper. When
\begin{equation*}
G=\Z_2,
\qquad
\alpha=0,
\end{equation*}
Dijkgraaf--Witten theory is just untwisted $\Z_2$ gauge theory. Chapter~\ref{ch:bf} already showed that the same theory appears as compact abelian BF theory at level $2$. The purpose of this section is to identify that topological field theory with the long-wavelength limit of Kitaev's toric code \cite{Kitaev2003}.
...
\begin{remark}[Scope and division of labor with Chapter~\ref{ch:toporder}]\label{rem:dw-toric-scope}
Section~\ref{sec:dw-toric} proves only the formal equivalence at the level of topological sectors. Chapter~\ref{ch:toporder} will do the condensed-matter work that we intentionally avoid here: it will study the toric-code Hamiltonian as a physical many-body system, derive its quasiparticle excitations and string operators, and interpret the resulting braiding data. The present chapter needs only the topological statement that the untwisted $\Z_2$ Dijkgraaf--Witten theory is the effective TQFT capturing that infrared sector.
\end{remark}

\section{Twisted Dijkgraaf--Witten theory as an inflow laboratory}\label{sec:dw-inflow}

The untwisted theory of Section~\ref{sec:dw-toric} was completely gauge-invariant on its own. Twisted Dijkgraaf--Witten theory is more subtle on manifolds with boundary: the bulk remains perfectly well defined on a closed manifold, but a nontrivial cocycle class forces any boundary realization to transform by a local phase. That is the finite-group version of anomaly inflow.
...
\item Chapter~\ref{ch:toporder} will use Section~\ref{sec:dw-toric} as the formal TQFT statement behind the toric-code phase before returning to the lattice Hamiltonian and emergent quasiparticles.
\item Chapter~\ref{ch:anyons} will use Section~\ref{sec:dw-action} for the finite-gauge-theory viewpoint on topological sectors, Section~\ref{sec:dw-zn} for the cyclic twists, and Section~\ref{sec:dw-inflow} for the untwisted-versus-twisted $\Z_2$ contrast.
\item Chapter~\ref{ch:outlook} will return to Chapter~\ref{ch:dw} as the baseline group-cohomology classification scheme before discussing gapped phases and anomalies that lie beyond that framework.
\end{itemize}
\end{remark}
