...
\title{The theoretical buildup to Chern--Simons theory}
\author{Brian Siew}
\date{\today}
...
\begin{abstract}
This paper develops Chern--Simons theory for a reader at the level of \cite{Schwartz2014QFT} through Chapter 30. We construct the action, prove that the identity $\tr(F\wedge F) = d\omega_{\CS}$ makes the Chern--Simons three-form into a natural object on three-manifolds, derive the gauge transformation law of $\omega_{\CS}$ by direct calculation, and deduce integer quantization of the level for $G=SU(2)$ from an explicit Euler-angle evaluation of $\int_{SU(2)}\tr((g^{-1}dg)^{\wedge 3}) = 24\pi^2$. We then describe the theory's observables (the partition function and Wilson loops), compute the abelian two-loop correlator $\langle W_{q_1}(C_1)W_{q_2}(C_2)\rangle_{S^3}$ in terms of the linking number, and canonically quantize the theory on $\R\times \Sigma$. For $G=U(1)$ and $\Sigma = T^2$ we derive $\dim \mathcal{H}_{U(1),k}(T^2) = k$ from the reduced torus phase space and a finite Heisenberg algebra. The presentation is pedagogical: differential-geometric machinery is developed only as needed, and every calculation is carried out in full.
\end{abstract}
...
\bibliography{chern_simons_theory}
